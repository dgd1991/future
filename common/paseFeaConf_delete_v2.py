...
def parse_feaconf(commonPath, modelName):
    ...
    kv_map = {}
    lineNum = -3
    if os.path.isfile(os.path.join(confPath, modelName + "_delete")):
        os.remove(os.path.join(confPath, modelName + "_delete"))
    new_config_file = open(os.path.join(confPath, modelName + "_delete"), "a")
    new_config_file.write("name | type | default_value | is_used | bucket | dim | shared_field | tower")
    ...
    with open(conf_file, "r") as fin:
        ...
        while line:
            ...
            fea_config = {}
            print(lineNum)
            name, type, default_value, is_used, bucket, dim, shared_field, tower = re.split("\|", line.strip())
            if is_used.strip() == '1' and fea_need_delete.__contains__(name.strip()):
                is_used = ' 0 '
            new_line = name + "|" + type + "|" + default_value + "|" + is_used + "|" + bucket + "|" + dim + "|" + shared_field + "|" + tower
            new_config_file.write("\n")
