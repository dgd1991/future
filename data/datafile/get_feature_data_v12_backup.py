...
class Feature(object):
   ...
   def feature_process(self):
      if self.is_code:
         ...
         raw_k_data = raw_k_data[raw_k_data['date']>=(str(year-1)+"-01-01")]
      ...
      if self.is_industry1:
         ...
         for day_cnt in [3, 5, 10, 20, 30, 60, 120]:
            ...
            if day_cnt == 3:
               tmp['code_count'] = tmp.groupby('date')['pctChg_rank' + str(day_cnt) + 'd'].transform('max')
            industry_id_level1_k_data['pctChg_rank_ratio' + str(day_cnt) + 'd'] = (tmp['pctChg_rank' + str(day_cnt) + 'd']/tmp['code_count'])
      ...
      if self.is_industry2:
        ...
        # 新增板块的最近n天的涨停股票数量，主要评估板块热度
         ...
         for day_cnt in [3, 5, 10, 20, 30, 60, 120, 240]:
            ...
            if day_cnt == 3:
               tmp['code_count'] = tmp.groupby('date')['pctChg_rank' + str(day_cnt) + 'd'].transform('max')
            industry_id_level2_k_data['pctChg_rank_ratio' + str(day_cnt) + 'd'] = (tmp['pctChg_rank' + str(day_cnt) + 'd']/tmp['code_count'])
      ...
      # 三级行业特征
        ...
        # 新增板块的最近n天的涨停股票数量，主要评估板块热度
         ...
         for day_cnt in [3, 5, 10, 20, 30, 60, 120]:
            ...
            if day_cnt == 3:
               tmp['code_count'] = tmp.groupby('date')['pctChg_rank' + str(day_cnt) + 'd'].transform('max')
            industry_id_level3_k_data['pctChg_rank_ratio' + str(day_cnt) + 'd'] = (tmp['pctChg_rank' + str(day_cnt) + 'd']/tmp['code_count'])
