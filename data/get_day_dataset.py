...
class DownloadDayDataSet(object):
    def __init__(self,
                 output_dir,
    ...
    def get_quarter_data_all(self):
        ...
        for index, row in stock_df.iterrows():
            ...
            quarter_data = self.get_quarter_data(code, get_date_previous_quarter(self.date_end.split('-')[0],self.date_end.split('-')[1], True))
            if quarter_data is not None:
                quarter_data_list.append(quarter_data)
        quarter_data_all = pd.concat(quarter_data_list)
        return quarter_data_all

    def get_feature_and_label(self, k_file_name, industry_file_name, quarter_file_name):
        df = self.get_k_raw_data()
        ...
        # 季度财报特征
        quarter_data_all = self.get_quarter_data_all()
