...
import os
import copy
from data.feature.feature_process import *
...
class Downloader(object):
    ...
    def get_k_raw_data(self):
        ...
        for index, row in future_df.iterrows():
            ...
            if len(code_k_data)>0:
                if os.path.isfile('/data/futuredata/train/code_k_data.csv'):
    ...
    def get_k_data(self):
        ...
        for index, row in future_df.iterrows():
            ...
            index += 1
            if index < 500:
                df_list.append(bs.query_history_k_data_plus(row["code"], self.fields, start_date=self.date_start, end_date=self.date_end, frequency="d", adjustflag="1").get_data())
            # df_list.append(bs.query_history_k_data_plus(row["code"], self.fields, start_date=self.date_start, end_date=self.date_end, frequency="d", adjustflag="1").get_data())
        ...
        # df["open"] = pd.to_numeric(df["open"], errors='coerce')
        # df["open"] = pd.to_numeric(df["open"], errors='coerce')
        # df_code.to_csv('{output_dir}/{code}.{code_name}.csv'.format(output_dir=self.output_dir, code=row["code"], code_name=row["code_name"]), index=False)
        ...
        quarter_data_all = self.get_quarter_data_all(2019, 2022)
        k_log_data_tmp = copy.deepcopy(feature_all[["date", "code"]])
        k_log_data_tmp['k_log_quarter'] = k_log_data_tmp['date'].map(lambda x: get_date_quarter(x.split('-')[0], x.split('-')[1], False))
        quarter_feature = pd.merge(k_log_data_tmp, quarter_data_all, how="left", left_on=['code', "k_log_quarter"], right_on=['code', 'pub_quarter_x'])
        print(quarter_feature[['date', 'pubDate_x']].head())
        feature_all[['roeAvg','npMargin','gpMargin','netProfit','epsTTM','MBRevenue','totalShare','liqaShare','NRTurnRatio','INVTurnRatio','CATurnRatio','AssetTurnRatio','YOYEquity','YOYAsset','YOYNI','YOYEPSBasic','YOYPNI','currentRatio','quickRatio','cashRatio','YOYLiability','liabilityToAsset','assetToEquity','CAToAsset','tangibleAssetToAsset','ebitToInterest','CFOToOR','CFOToNP','CFOToGr','dupontROE','dupontAssetStoEquity','dupontAssetTurn','dupontPnitoni','dupontNitogr','dupontTaxBurden','dupontIntburden','dupontEbittogr']] = quarter_feature.apply(lambda x: [x.roeAvg_x,x.npMargin_x,x.gpMargin_x,x.netProfit_x,x.epsTTM_x,x.MBRevenue_x,x.totalShare_x,x.liqaShare_x,x.NRTurnRatio_x,x.INVTurnRatio_x,x.CATurnRatio_x,x.AssetTurnRatio_x,x.YOYEquity_x,x.YOYAsset_x,x.YOYNI_x,x.YOYEPSBasic_x,x.YOYPNI_x,x.currentRatio_x,x.quickRatio_x,x.cashRatio_x,x.YOYLiability_x,x.liabilityToAsset_x,x.assetToEquity_x,x.CAToAsset_x,x.tangibleAssetToAsset_x,x.ebitToInterest_x,x.CFOToOR_x,x.CFOToNP_x,x.CFOToGr_x,x.dupontROE_x,x.dupontAssetStoEquity_x,x.dupontAssetTurn_x,x.dupontPnitoni_x,x.dupontNitogr_x,x.dupontTaxBurden_x,x.dupontIntburden_x,x.dupontEbittogr_x] if str(x.date) > str(x.pubDate_x) else [x.roeAvg_y,x.npMargin_y,x.gpMargin_y,x.netProfit_y,x.epsTTM_y,x.MBRevenue_y,x.totalShare_y,x.liqaShare_y,x.NRTurnRatio_y,x.INVTurnRatio_y,x.CATurnRatio_y,x.AssetTurnRatio_y,x.YOYEquity_y,x.YOYAsset_y,x.YOYNI_y,x.YOYEPSBasic_y,x.YOYPNI_y,x.currentRatio_y,x.quickRatio_y,x.cashRatio_y,x.YOYLiability_y,x.liabilityToAsset_y,x.assetToEquity_y,x.CAToAsset_y,x.tangibleAssetToAsset_y,x.ebitToInterest_y,x.CFOToOR_y,x.CFOToNP_y,x.CFOToGr_y,x.dupontROE_y,x.dupontAssetStoEquity_y,x.dupontAssetTurn_y,x.dupontPnitoni_y,x.dupontNitogr_y,x.dupontTaxBurden_y,x.dupontIntburden_y,x.dupontEbittogr_y], axis=1)

        print(feature_all.head())
        # print(label.iloc[800:802])
        return feature_all, label

    # def read_history_data(self):
    #     raw_data = pd.read_csv("/Users/admin/PycharmProjects/future/data/futuredata/train/sz.399998.中证煤炭指数.csv")
        ...
        for index, row in future_df.iterrows():
            ...
            if index > 2012:
                ...
                if not quarter_data_list == []:
                    ...
                    if index == 1:
                        quarter_data_tmp.to_csv('{output_dir}/{quarter_data}.csv'.format(output_dir=self.output_dir, quarter_data='quarter_data1'), mode='a', header=True, index=False)
                    else:
                        quarter_data_tmp.to_csv('{output_dir}/{quarter_data}.csv'.format(output_dir=self.output_dir, quarter_data='quarter_data1'), mode='a', header=True, index=False)
...
if __name__ == '__main__':
    mkdir('./futuredata/train')
    downloader = Downloader('./futuredata/train', date_start='2019-01-01', date_end='2022-04-21')
    ...
    #
    # mkdir('./futuredata/test')
    # downloader = Downloader('./futuredata/test', date_start='2022-03-01', date_end='2022-03-11')
    # downloader.run()