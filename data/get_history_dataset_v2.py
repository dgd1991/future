...
class Downloader(object):
    def __init__(self,
        ...
        self.code_k_data_file_name = 'code_k_data_v2_2006'
        self.code_quarter_data_file_name = 'code_quarter_data_v2_2006'
        ...
        self.code_industry_data = self.get_industry_data(date_end)
    ...
    def get_quarter_data_all(self):
        ...
        if os.path.isfile(file_dir_name):
            ...
            quarter_data_his_2 = quarter_data_his[(quarter_data_his['quarter'] == str(year_2) + str(quarter_2))]
            quarter_data_his = quarter_data_his[(quarter_data_his['quarter'] == str(year) + str(quarter))]
            quarter_data_his = pd.merge(self.future_df, quarter_data_his, how="left", left_on=['code'],right_on=['code'])
            quarter_data_his = quarter_data_his[quarter_data_his['quarter'] == None]
            quarter_data_his_2 = pd.merge(self.future_df, quarter_data_his_2, how="left", left_on=['code'], right_on=['code'])
            quarter_data_his_2 = quarter_data_his[quarter_data_his_2['quarter'] == None]
        else:
...
if __name__ == '__main__':
    # mkdir('./datafile')
    import datetime
    begin = datetime.date(2006, 1, 2)
    end = datetime.date(2006, 12, 31)
    d = begin
    delta = datetime.timedelta(days=1)
    while d <= end:
        print (d.strftime("%Y-%m-%d"))
        downloader = Downloader('E:/pythonProject/future/data/datafile', date_start=d.strftime("%Y-%m-%d"), date_end=d.strftime("%Y-%m-%d"))
        ...
        downloader.exit()
        d += delta
    # downloader = Downloader('E:/pythonProject/future/data/datafile', date_start='2022-06-16', date_end='2022-06-16')
    # if len(downloader.future_df)>0:
    #     downloader.get_k_raw_data()
    #     # downloader.get_quarter_data_all()
    #     # downloader.get_feature_and_label(k_file_name, industry_file_name, quarter_file_name)
